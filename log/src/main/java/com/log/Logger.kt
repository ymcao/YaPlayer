...
object Logger {
    ...
    @JvmStatic fun json(map: Map<*, *>?) {
        if (map != null) {
            ...
            try {
                ...
                var message = jsonObject.toString(Printer.JSON_INDENT)
    ...
    @JvmStatic fun json(json: String?) {
        ...
        try {
            ...
            if (json.startsWith("[")) {
                ...
                var message = jsonArray.toString(Printer.JSON_INDENT)
                message = message.replace("\n".toRegex(), "\n║ ")
                val s = getMethodNames()
                println(String.format(s, message))
                return
    ...
    fun getMethodNames(): String {
        ...
        var stackOffset = Printer.getStackOffset(sElements)

        stackOffset++
        val builder = StringBuilder()
        builder.append(Printer.TOP_BORDER).append("\r\n")
                // 添加当前线程名
                .append("║ " + "Thread: " + Thread.currentThread().name).append("\r\n")
                .append(Printer.MIDDLE_BORDER).append("\r\n")
                // 添加类名、方法名、行数
                .append("║ ")
                ...
                .append(Printer.BOTTOM_BORDER).append("\r\n")
